...
public class TalonFXSSwerve extends SwerveMotor
{
  ...
  {
    // Only supports CANcoder
    cfg.refresh(configuration);
    this.absoluteEncoder = false;
    try {
      CANcoder maybeEncoder = (CANcoder) encoder.getAbsoluteEncoder();
      ExternalFeedbackConfigs encoderConfig = configuration.ExternalFeedback
      .withAbsoluteSensorDiscontinuityPoint(1)
      .withRemoteCANcoder(maybeEncoder);
      cfg.apply(encoderConfig);
